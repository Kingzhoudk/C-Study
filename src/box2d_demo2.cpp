...
int main() {
  ...
  ball.position.Set(1.5f, 4.0f);
  b2Body* ball_body = world.CreateBody(&ball);

  b2CircleShape ball_shape;
  b2Vec2        point3(1.5f, 4.0f);
  //ballShape.m_p = point3;    //设定坐标
  ball_shape.m_radius = 0.1;  //设定半径

  b2FixtureDef ball_fixtureDef;
  ball_fixtureDef.shape = &ball_shape;  //shape为圆
  ...
  // 创建刚体
  b2BodyDef sensor;
  sensor.type = b2_staticBody;
  b2Body* sensor_body = world.CreateBody(&sensor);
  // 创建形状
  ...
  sensor_def.shape = &sensor_shape;
  sensor_body->CreateFixture(&sensor_def);
