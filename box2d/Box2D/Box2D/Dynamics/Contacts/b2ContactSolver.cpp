	...
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		if (pointCount == 1 || g_blockSolve == false)
		{
			for (int32 j = 0; j < pointCount; ++j)
			{
				...
				float32 newImpulse = b2Max(vcp->normalImpulse + lambda, (float32)0.0f);
...
bool b2ContactSolver::SolvePositionConstraints()
{
	...
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		for (int32 j = 0; j < pointCount; ++j)
		{
			...
			float32 C = b2Clamp(b2_baumgarte * (separation + b2_linearSlop), -b2_maxLinearCorrection, (float32)0.0f);
...
bool b2ContactSolver::SolveTOIPositionConstraints(int32 toiIndexA, int32 toiIndexB)
{
	...
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		for (int32 j = 0; j < pointCount; ++j)
		{
			...
			// Prevent large corrections and allow slop.
			float32 C = b2Clamp(b2_toiBaugarte * (separation + b2_linearSlop), -b2_maxLinearCorrection, (float32)0.0f);
